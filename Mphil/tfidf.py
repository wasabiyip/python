...
class TfIdf:
  ...
  def add_input_document(self, input):
	...
	##print self.term_num_docs[word]
      else:
        self.term_num_docs[word] = 1
	self.voc[word]=self.ID
        self.ID=self.ID + 1
  ...
  def get_idf(self, term):
   ...
   # print self.get_num_docs()
    #print self.term_num_docs[term]
    return math.log10(float( self.get_num_docs()) / 
      ( self.term_num_docs[term]))
  ...
  def get_doc_keywords(self, curr_doc):
    ...
    #  mytf = float(tokens.count(word)) / len(tokens_set)
     ...
     # print mytf
      ...
      newtfidf[self.voc[word]] = mytf * myidf
     # return sorted(newtfidf.items(), key=itemgetter(1), reverse=True)
###
    #return sorted(tfidf.items(), key=itemgetter(1), reverse=True)
    return sorted(newtfidf.items(), key=itemgetter(0), reverse=False)
